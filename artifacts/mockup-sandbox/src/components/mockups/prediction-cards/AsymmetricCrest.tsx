...
function MatchCardCrest({ match }: { match: Match }) {
  ...
  return (
    <button
      ...
      className="tt-card relative block w-full text-left pl-5 pr-4 py-4 group overflow-hidden"
    >
      {/* lime side stripe */}
      <span
        className={`absolute left-0 top-4 bottom-4 w-[3px] rounded-r-full ${
          won ? "tt-bg-lime" : lost ? "bg-red-400/50" : isLive ? "tt-bg-lime tt-pulse" : "bg-white/10"
        }`}
      />
      ...
      {/* asymmetric crest — overlapped logos LEFT, names stacked RIGHT */}
      <div className="flex items-center gap-3.5 mb-4">
        <div className="relative w-[58px] h-11 flex-shrink-0">
          <div className="tt-logo-plate absolute left-0 top-0 w-11 h-11 rounded-full flex items-center justify-center z-10 ring-2 ring-[#131316]">
            <img src={match.home_logo} alt={match.home_team} className="w-7 h-7 object-contain" />
          </div>
          <div className="tt-logo-plate absolute right-0 top-0 w-9 h-9 mt-1 rounded-full flex items-center justify-center ring-2 ring-[#131316]">
            <img src={match.away_logo} alt={match.away_team} className="w-5 h-5 object-contain" />
          </div>
        </div>

        <div className="flex-1 min-w-0">
          <p className="text-[14.5px] text-white leading-[1.15] font-medium truncate">
            {match.home_team}
          </p>
          <p className="text-[12.5px] text-white/50 leading-[1.15] truncate mt-0.5">
            vs <span className="text-white/70">{match.away_team}</span>
          </p>
        </div>

        {(isLive || isFinished) && (
          <div className="tt-display tt-mono tt-tabular text-[20px] text-white leading-none flex-shrink-0 pr-1">
            {match.home_score}
      ...
      {/* bet block */}
      <div className="tt-bt-hairline pt-3.5 flex items-end justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <div className="tt-display text-[17.5px] text-white leading-tight truncate font-medium">
            {bet.bet_type}
            <span className={lost ? "text-red-400/80" : "tt-text-lime"}>.</span>
          </div>
        </div>

        <div className="flex items-end gap-3.5 flex-shrink-0">
...
export function AsymmetricCrest() {
  return (
    <div className="tt-page p-4">
      <div className="mb-5">
        ...
        <p className="text-[11px] text-white/40 tracking-[0.06em]">
          Yön 3 — Asymmetric Crest
        </p>
      </div>

      <div className="space-y-3">
        {matches.map((m) => (
          <MatchCardCrest key={m.fixture_id} match={m} />
        ))}
      </div>
    </div>
  );
