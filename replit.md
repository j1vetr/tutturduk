...
- **Central Config**: All thresholds live in `server/predictionConfig.ts` (`MAX_DAILY_MATCHES=40`, `MIN_STATS_SCORE=35`, `MIN_CONF=70`, `MIN_VALUE=0.05`, `MIN_ODDS=1.40`, sweet-spot 1.55–2.20, AI batch size 5, multi-pass schedule via `PUBLISH_PASSES`).
...
- **Multi-Pass Daily Pipeline (cron Türkiye saati 01:00 / 10:00 / 14:00 / 17:00)**: Gün boyu 4 geçişle yayın yapılır — 01:00 anchor (tüm gün), 10:00/14:00/17:00 incremental passes (`futureOnlyMinutes=5` ile o saatten 5 dk sonra başlayacak maçlar) + cache temizliği (`refreshCache=true`) → taze API çağrısı. Her pass: **önce Postgres advisory lock** (`pg_try_advisory_lock`, key=73572404) → cache temizlik → fixture prefetch → DB count check (40 cap) → odds early-elim (sweet spot) → top-N by stats score + buffer → parallel AI batch (5×) → `INSERT ... ON CONFLICT DO NOTHING RETURNING id` → lock'u `finally`'de bırak. Lock cache silme + API çağrısından önce alındığı için paralel pass'ler birbirini ezemez. Tüm geçişlerin toplamı **40 cap atomik** korunur. Pass tanımları `predictionConfig.PUBLISH_PASSES` üzerinden yönetilir; `runScheduledPublishPass(dateStr, opts)` tek paylaşılan implementasyondur.
...
- **Coupon Automation**: `autoCreateDailyCoupon()` picks top 2–3 primary bets; bets with missing/invalid odds are excluded — coupon is rolled back if fewer than 2 valid bets remain. `startCouponSchedulerService` her gün **18:30** Türkiye saatinde çalışır (Pass 4 bittikten sonra) ve aynı `PUBLISH_LOCK_KEY` advisory lock'unu test eder; pass hâlâ çalışıyorsa 60s aralıklarla 3 kez retry yapar.
- **Manuel Tetikleme Future-Only Filtresi**: `/api/admin/auto-publish-today` ve `publishFromPrefetchedFixtures` artık opsiyonel `futureOnlyMinutes` (default 5) alır. Bugün için tetiklendiğinde kickoff'u `now + 5 dk`'dan önce olan maçlar otomatik elenir; örn. 20:00'de tetiklenirse 18:00'de başlamış maçlar atlanır, 20:05+ başlayanlar analiz edilir. Yarın tetiklemesi (`/api/admin/auto-publish`) her zaman `futureOnlyMinutes=null` (filtre yok) ile çalışır.
